...
def tinh_chi_so_danh_gia(anh_goc, anh_nhung, anh_nuoc, anh_khoi_phuc):
    ...
    return {"PSNR": psnr, "SSIM": ssim, "Pearson": rho, "ASR": asr}

# Hàm minh họa với ma trận 4x4
def minh_hoa_ma_tran_4x4():
    ...
    ma_tran_nhung = np.clip(ma_tran_nhung, 0, 255)
    ...
    for _ in range(SO_VONG_LAP_TOI_DA):
        ...
        nhieu_tich_cuc[i, j] += BIEN_DO_NHIEU
        nhieu_tieu_cuc[i, j] -= BIEN_DO_NHIEU
        nhieu_tich_cuc = np.clip(nhieu_tich_cuc, 0, 255)
        nhieu_tieu_cuc = np.clip(nhieu_tieu_cuc, 0, 255)
    ...
    return (ma_tran_goc, ma_tran_nuoc, ma_tran_nhung, psnr, ssim, rho, asr)
...
class UngDungNuoc:
    ...
    def setup_trich_xuat_tab(self, tab):
        ...
        Button(main_frame, text="Trích xuất", command=self.trich_xuat, font=("Arial", 12), bg="#4CAF50", fg="white", width=10).pack(pady=10)
    ...
    def chay_demo(self):
        try:
            ma_tran_goc, ma_tran_nuoc, ma_tran_nhung, psnr, ssim, rho, asr = minh_hoa_ma_tran_4x4()
